...
class _BarberDashboardScreenState extends ConsumerState<BarberDashboardScreen> {
    ...
    try {
      if (user.role == AppRole.owner) {
        final results = await Future.wait([
          ...
          apiService.getStaffProfile(user.id), // Try to get staff profile for owner
        ]);
        
        _analytics = results[0] as Map<String, dynamic>?;
        final shops = results[1] as List<dynamic>;
        _staffProfile = results[2] as Map<String, dynamic>?; // Nullable if not found
  ...
  Widget _buildScreenHeader(bool isDark, {required String title, String? subtitle, Widget? trailing, bool showAura = false, bool isOnline = true, VoidCallback? onToggleOnline}) {
                        ...
                        Padding(
                          ...
                          child: InkWell(
                            ...
                            child: Container(
                              ...
                              decoration: BoxDecoration(
                                color: isOnline ? AppTheme.emerald.withOpacity(0.1) : Colors.red.withOpacity(0.1),
                                borderRadius: BorderRadius.circular(20),
                                border: Border.all(color: isOnline ? AppTheme.emerald : Colors.red, width: 1.5),
                              ),
                              child: Row(
                                ...
                                children: [
                                  Container(
                                    width: 8, height: 8,
                                    decoration: BoxDecoration(
                                      color: isOnline ? AppTheme.emerald : Colors.red,
                                      shape: BoxShape.circle,
                                    ),
                                  ),
                                  ...
                                  Icon(
                                    isOnline ? LucideIcons.toggleRight : LucideIcons.toggleLeft, 
                                    size: 16, 
                                    color: isOnline ? AppTheme.emerald : Colors.red
                                  ),
                                ],
  ...
  Future<void> _toggleOnlineStatus() async {
    ...
    final currentStatus = _staffProfile!['isAvailable'] ?? true;
    final newStatus = !currentStatus;
    
    // Optimistic Update
    setState(() {
      _staffProfile!['isAvailable'] = newStatus;
    });

    try {
      final apiService = ref.read(apiServiceProvider);
      await apiService.updateStaffProfile(_staffProfile!['id'], {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            ...
            backgroundColor: newStatus ? AppTheme.emerald : Colors.red,
      ...
      if (mounted) {
        setState(() {
          _staffProfile!['isAvailable'] = currentStatus;
        });
  ...
  Widget _buildDashboardTab(bool isDark, int unreadCount) {
    ...
    final hasStaffProfile = _staffProfile != null;
    ...
    return Column(
      ...
      children: [
        _buildScreenHeader(
          ...
          showAura: hasStaffProfile, // Show aura/toggle if staff profile exists
          isOnline: (_staffProfile != null) ? (_staffProfile!['isAvailable'] ?? true) : true,
          onToggleOnline: hasStaffProfile ? _toggleOnlineStatus : null, // Enable toggle if staff profile exists
        ),
      ...
      child: ClipRRect(
        ...
        child: BackdropFilter(
          ...
          child: Container(
            padding: const EdgeInsets.all(20),
            decoration: BoxDecoration(
              color: isDark ? AppTheme.darkCardBG.withOpacity(0.4) : focusColor.withOpacity(0.1),
              borderRadius: BorderRadius.circular(24),
                  ...
                  _buildProfileItem(
                    ...
                    onTap: () {
                       if (_staffProfile != null && _staffProfile!['shop'] != null) {
                          final shopData = Map<String, dynamic>.from(_staffProfile!['shop']);
                          shopData['isAvailable'] = _staffProfile!['isAvailable']; // Pass manual status
                          context.push('/shop-preview', extra: shopData); 
                       } else {
                  ...
                  _buildProfileItem(
                    ...
                    onTap: () async {
                      if (_shopProfile != null) {
                        final shopData = Map<String, dynamic>.from(_shopProfile!);
                        // Inject availability status for accurate preview inside settings
                        if (_staffProfile != null) {
                          shopData['isAvailable'] = _staffProfile!['isAvailable'];
                        }
                        await context.push('/shop-settings', extra: shopData); 
                        _loadData();
                      }
                    }
                  ),
